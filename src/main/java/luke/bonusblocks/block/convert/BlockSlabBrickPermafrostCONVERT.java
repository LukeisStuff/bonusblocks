...
public class BlockSlabBrickPermafrostCONVERT extends BlockSlab {
    ...
    public void onBlockPlaced(World world, int x, int y, int z, Side side, EntityLiving entity, double sideHeight) {
        ...
        if (dir == Direction.DOWN) {
            world.setBlockAndMetadataWithNotify(x, y, z, Block.slabBrickPermafrost.id, 0);
        }

        if (dir == Direction.UP) {
            world.setBlockAndMetadataWithNotify(x, y, z, Block.slabBrickPermafrost.id, 2);
        }

    }

    public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
        return new ItemStack[]{new ItemStack(Block.slabBrickPermafrost)};
    }
