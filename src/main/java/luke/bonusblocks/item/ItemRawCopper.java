...
public class ItemRawCopper extends Item {
    ...
    public ItemRawCopper(String name, String namespaceId, int id) {
    ...
    public boolean onUseItemOnBlock(ItemStack itemstack, Player entityplayer, World world, int blockX, int blockY, int blockZ, Side side, double xPlaced, double yPlaced) {
        ...
        if (itemstack.stackSize <= 0) {
            ...
            int newMeta;
            if (id == BonusBlocks.OVERLAY_RAW_COPPER.id && side == Side.TOP) {
                newMeta = meta + 1;
                ...
                if (newMeta < 3) {
                    world.setBlockAndMetadataWithNotify(blockX, blockY, blockZ, BonusBlocks.OVERLAY_RAW_COPPER.id, newMeta);
                    world.playBlockSoundEffect(entityplayer, (float)blockX + 0.5F, (float)blockY + 0.5F, (float)blockZ + 0.5F, BonusBlocks.OVERLAY_RAW_COPPER, EnumBlockSoundEffectType.PLACE);
            ...
            if (id == BonusBlocks.OVERLAY_RAW_COPPER.id) {
                newMeta = meta + 1;
            ...
            if (world.canBlockBePlacedAt(BonusBlocks.OVERLAY_RAW_COPPER.id, blockX, blockY, blockZ, false, side) && world.isBlockOpaqueCube(blockX, blockY - 1, blockZ) && world.setBlockAndMetadataWithNotify(blockX, blockY, blockZ, BonusBlocks.OVERLAY_RAW_COPPER.id, 0)) {
                BonusBlocks.OVERLAY_RAW_COPPER.onBlockPlacedByMob(world, blockX, blockY, blockZ, side, entityplayer, yPlaced);
                world.playBlockSoundEffect(entityplayer, (float)blockX + 0.5F, (float)blockY + 0.5F, (float)blockZ + 0.5F, BonusBlocks.OVERLAY_RAW_COPPER, EnumBlockSoundEffectType.PLACE);
                itemstack.consumeItem(entityplayer);
                return true;
            } else {
